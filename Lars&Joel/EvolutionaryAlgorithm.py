...
def evolve(data, label: str, maxIterations = 50, popSize = 20, fitnessIter = 2):
    population = {}
    population["genomes"] = createRandomPopulation(popSize,2,8,4,128,512)
    iter = 0
    while maxIterations > iter:
        print(f"Iteration {iter}")
        population["fitness"] = [fitness(genome, data, label, fitnessIter) for genome in population["genomes"]]
        sorted_population = [x for _, x in sorted(zip(population["fitness"], population["genomes"]), reverse=True)]
...
def createRandomPopulation(sizeOfPupulation: int, minLayers: int, maxLayers: int, minNeurons: int, maxNeurons: int, maxTotalNumberOfNeurons: int = 0):
    population = [createRandomGenome(minLayers, maxLayers, minNeurons, maxNeurons, maxTotalNumberOfNeurons) for _ in range(sizeOfPupulation)]
    return population
...
def createRandomGenome(minLayers: int, maxLayers: int, minNeurons: int, maxNeurons: int, maxTotalNumberOfNeurons: int = 0):
    ...
    Args:
        ...
        maxTotalNumberOfNeurons (int): The maximal number of neurons in all layers. If 0 is ignored
...
def fitness(genome: [], data, label: str, numOfIterations: int = 5) -> float:
...
def createChildren(parents, numChildren):
    ...
    while len(children) <= numChildren:
        ...
        if parent1 != parent2: # If the parents are the same entity a clone would be created
    ...
    if random.random() < mutationChance:
        ...
        if random.random() < 0.25: # Mutate a layer
            ...
            if r < 0.33:
                ...
                genome[i] += random.randint(1, maxNeurons - genome[i])
            elif genome[i] > 4:
